# bot/site_builder.py
"""
Senior News Daily — Clean Site Builder (with weekly trends)
------------------------------------------------------------
Builds:
- site/index.html (hero, summary, trends, categories, alerts, archives)
- site/scams.html (resources)
- site/saved.html
- site/archive/YYYY-MM-DD.html + index.html
- site/styles.css
"""
...
def next_update_badge(now: dt | None = None) -> str:
    """Show approximate time until the next GitHub Action run (00:00 & 12:00 UTC)."""
...
def weekly_trends(items: list[dict]) -> str:
    ...
    if not counts:
        return "<p class='muted'>No articles in the last 7 days.</p>"
    top = counts.most_common(8)
    maxv = max(v for _,v in top)
    rows=[]
    for cat,v in top:
        slug=slugify(cat)
        rows.append(f"<div class='trend'><b>{esc(cat)}</b><div class='bar'><span style='width:{int(v/maxv*100)}%;background:var(--cat-{slug},#2563eb)'></span></div><div class='val'>{v}</div></div>")
    return "<div class='trends'>"+"\n".join(rows)+"</div>"

# ---------------------- Components ----------------------
def render_card(it):
    cat = it["category"]; slug = slugify(cat)
    return (f"<div class='card cat-{slug}' data-id='{esc(it.get('id',''))}' data-cat='{slug}'>"
...
def build_archive_pages(items):
    ...
    for d,its in by_day.items():
        (ARCH/f"{d}.html").write_text(
            ...
            f"<footer class='footer'><p>Venmo: <strong>@MikeHnastchenko</strong></p></footer>",encoding='utf-8')
    def list_html(prefix): 
        return "<ul class='archives'>"+"".join(
            f"<li><a href='{prefix}{d}.html'>{d}</a> <span class='muted'>({len(v)} articles)</span></li>"
            for d,v in sorted(by_day.items(),reverse=True))+"</ul>"
...
<li><a href='https://oig.ssa.gov/report/' target='_blank'>SSA OIG: Report Fraud</a></li>
...
<p class='muted'>© __YEAR__ Senior News Daily</p></footer>
<script>
...
arch_index=f"<!doctype html><meta charset='utf-8'><link rel='stylesheet' href='../styles.css'><div class='topnav'><div class='container'><a href='../index.html'>← Home</a><div class='muted'>Archive</div></div></div><main class='container'><h1>Archive</h1>{archives_index}</main><footer class='footer'><p>Venmo: <strong>@MikeHnastchenko</strong></p></footer>"
(ARCH/"index.html").write_text(arch_index,encoding="utf-8")
(SITE/"saved.html").write_text("<!doctype html><meta charset='utf-8'><title>Saved</title><link rel='stylesheet' href='styles.css'><div class='topnav'><div class='container'><a href='index.html'>← Home</a></div></div><main class='container'><h1>Saved Articles</h1><div id='savedCards'><p class='muted'>Loading…</p></div></main><footer class='footer'><p>Venmo: <strong>@MikeHnastchenko</strong></p></footer><script>const savedIds=new Set(JSON.parse(localStorage.getItem('snd_saved')||'[]'));fetch('index.html').then(r=>r.text()).then(t=>{const doc=new DOMParser().parseFromString(t,'text/html');const cards=[...doc.querySelectorAll('.card')].filter(c=>savedIds.has(c.dataset.id));document.querySelector('#savedCards').innerHTML=cards.length?cards.map(c=>c.outerHTML).join(''):'<p>No saved articles yet.</p>';});</script>",encoding="utf-8")

print(f"[site_builder] Built site with {len(items)} articles across {len(cat_counts)} categories.")
