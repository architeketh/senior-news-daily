...
ROOT = pathlib.Path("."); DATA = ROOT / "data"; DATA.mkdir(exist_ok=True)
ITEMS = DATA / "items.json"; OUT = DATA / "sources.json"
...
def norm(rec: Any) -> Optional[Dict[str, Any]]:
    ...
    dtx  = (dtparse(rec.get("published")) or dtparse(rec.get("pubDate")) or dtparse(rec.get("isoDate")) or
        ...
        for k in ("items","articles","entries","results","data"):
            ...
            if isinstance(v, list):
                for x in v:
                    n = norm(x);  if n: yield n
                return
        n = norm(container);  if n: yield n

raw = load_json(ITEMS, [])
items: List[Dict[str, Any]] = list(iter_items(raw))
now = datetime.datetime.utcnow().replace(tzinfo=datetime.timezone.utc)
window_start = now - datetime.timedelta(days=7)
def safe_dt(it: Dict[str, Any]) -> datetime.datetime: return it.get("_dt") or now
curr = [a for a in items if safe_dt(a) >= window_start]

stats: Dict[str, Dict[str, Any]] = {}
for a in curr:
    dom = domain(a.get("link","")); key = (a.get("source") or dom or "unknown").lower()
    ts = safe_dt(a)
    s = stats.setdefault(key, {"display": a.get("source") or dom or "unknown", "domain": dom or "unknown",
                               "count": 0, "last_dt": ts, "last_title": a.get("title",""), "last_link": a.get("link","")})
    s["count"] += 1
    if ts > s["last_dt"]: s["last_dt"]=ts; s["last_title"]=a.get("title",""); s["last_link"]=a.get("link","")
